...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await query(
      "UPDATE geburtstage SET datum=$1, name=$2, inhalt=$3, old=$4, month_day=$5, adresse=$6, gruppe=$7 WHERE id=$8",
      [datum, name.trim(), inhalt.trim(), age, monthDay, adresse.trim(), gruppe.trim(), numId]
    );

    return NextResponse.json({ id, Datum: datum, Name: name.trim(), Inhalt: inhalt.trim(), Old: age, Adresse: adresse.trim(), Gruppe: gruppe.trim() });
  } catch (err) {
    console.error(err);
